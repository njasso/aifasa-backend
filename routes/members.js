...
// Nous utilisons un middleware upload.none() pour les routes qui n'ont pas d'upload de fichier
// et upload.fields() pour celles qui en ont.
...
    // Nettoyer les fichiers temporaires si l'accès est refusé
    if (req.files) {
      const filesToClean = [
...
  // Extraction des données du corps et des fichiers
  const {
...
        resource_type: 'raw' // Très important pour les documents
      });
...
  } finally {
    // Suppression des fichiers temporaires dans tous les cas
    if (req.files) {
      const filesToClean = [
        ...(req.files.profilePicture || []),
        ...(req.files.cv || [])
...
    // Logique de mise à jour pour la photo de profil
    if (profilePictureFile) {
...
      public_id = result.public_id;
    }
    // Logique de mise à jour pour le CV
    if (cvFile) {
